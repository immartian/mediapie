...
class OMXPlayer(object):
	...
	# Get file properties
	result = self._process.readline()
	file_props = self._FILEPROP_REXP.match(result).groups()
	(self.audio['streams'], self.video['streams'],
	 self.chapters, self.subtitles) = [int(x) for x in file_props]
	# Get video properties
	v = self._VIDEOPROP_REXP.match(result)
	if v is not None: 
		video_props  = v.groups()
	...
	a = self._AUDIOPROP_REXP.match(result)
	if a is not None:	
